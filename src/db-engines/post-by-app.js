...
module.exports = async function (op, timestamp) {
    ...
    if (op[1].parent_author === '') {  // check if its a post - not a comment
        ...
        try {
            ...
            if (post_app && post_app.indexOf(config.target) > -1) {
            ...
            console.log(err); //silence this error since technically its not an err - not all posts have 'app' value in their json_metadata
        }
    }
